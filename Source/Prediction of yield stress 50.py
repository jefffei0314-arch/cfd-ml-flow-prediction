...
# =========================================================
# 11. Engineering evaluation metrics
# =========================================================

# ---------- Front position ----------
threshold = 0.01

def get_front_position(x, h, threshold=0.01):
    idx = np.where(h < threshold)[0]
    if len(idx) > 0:
        return x[idx[0]]
    return x[-1]

front_cfd = get_front_position(df_50["x_ratio"].values, df_50["h_ratio"].values, threshold)
front_poly = get_front_position(df_50["x_ratio"].values, h_poly_interp, threshold)
front_rf = get_front_position(df_50["x_ratio"].values, h_rf_interp, threshold)

print("\nFront Position:")
print(f"CFD        = {front_cfd:.3f}")
print(f"Polynomial = {front_poly:.3f}  (Error = {front_poly - front_cfd:.3f})")
print(f"RF         = {front_rf:.3f}  (Error = {front_rf - front_cfd:.3f})")

# ---------- Active region RMSE ----------
active_mask = df_50["h_ratio"] > threshold

rmse_poly_active = np.sqrt(mean_squared_error(
    df_50.loc[active_mask, "h_ratio"],
    h_poly_interp[active_mask]
))

rmse_rf_active = np.sqrt(mean_squared_error(
    df_50.loc[active_mask, "h_ratio"],
    h_rf_interp[active_mask]
))

mae_poly_active = np.mean(np.abs(
    h_poly_interp[active_mask] - df_50.loc[active_mask, "h_ratio"]
))

mae_rf_active = np.mean(np.abs(
    h_rf_interp[active_mask] - df_50.loc[active_mask, "h_ratio"]
))

print("\nActive Region Performance (h_ratio > 0.01):")
print(f"Polynomial RMSE = {rmse_poly_active:.4f}")
print(f"Polynomial MAE  = {mae_poly_active:.4f}")
print(f"RF RMSE         = {rmse_rf_active:.4f}")
print(f"RF MAE          = {mae_rf_active:.4f}")

# ---------- Area under curve ----------
def my_trapz(y, x):
    return np.sum((y[:-1] + y[1:]) * (x[1:] - x[:-1]) / 2)

area_cfd = my_trapz(df_50["h_ratio"].values, df_50["x_ratio"].values)
area_poly = my_trapz(h_poly_interp, df_50["x_ratio"].values)
area_rf   = my_trapz(h_rf_interp, df_50["x_ratio"].values)

print("\nArea Under Curve (Volume Proxy):")
print(f"CFD        = {area_cfd:.4f}")
print(f"Polynomial = {area_poly:.4f}  (Error = {area_poly - area_cfd:.4f})")
print(f"RF         = {area_rf:.4f}  (Error = {area_rf - area_cfd:.4f})")

# =========================================================
# 12. Plot: CFD vs Polynomial vs Random Forest
# =========================================================
